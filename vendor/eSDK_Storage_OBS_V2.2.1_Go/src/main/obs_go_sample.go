...
func listBuckets() {
	output, err := getObsClient().ListBuckets(nil)
	if err == nil {
		fmt.Printf("StatusCode:%d, RequestId:%s\n", output.StatusCode, output.RequestId)
...
func setBucketPolicy() {
	...
	input.Bucket = bucketName
	input.Policy = "{\"Version\":\"2008-10-17\", \"Id\": \"Policy1375342051334\", \"Statement\": [{\"Sid\": \"Stmt1375240018061\", \"Action\": [\"s3:GetBucketPolicy\"]," +
		"\"Effect\": \"Allow\", \"Resource\": \"arn:aws:s3:::bucket-test\", \"Principal\": { \"AWS\": [\"*\"] } }]}"
...
func copyObject() {
	...
	input.Bucket = bucketName
	input.Key = objectKey
	input.CopySourceBucket = "source-bucket"
	input.CopySourceKey = "source-key"
